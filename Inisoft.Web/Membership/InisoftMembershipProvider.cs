...
{
    public sealed class InisoftMembershipProvider : MembershipProvider
    {

        IUserRepository _userProvider = RepositoryServiceLocator.Get<IUserRepository>();

        //
        // Global connection string, generated password length, generic exception message, event log info.
        //
        private string eventSource = "SmartMembershipProvider";
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (name == null || name.Length == 0)
                name = "SmartMembershipProvider";

            if (String.IsNullOrEmpty(config["description"]))
            {
                config.Remove("description");
                config.Add("description", "Smart Membership provider");
            }
            ...
            pPasswordStrengthRegularExpression = Convert.ToString(GetConfigValue(config["passwordStrengthRegularExpression"], string.Empty));
        ...
        private bool saveUser(User user, string source)
        {
            bool _result = false;
            try
            {
                _result = _userProvider.Save(user, Engine.DefaultUser).Success;
            }
            catch (Exception e)
            {
                if (WriteExceptionsToEventLog)
                {
                    WriteToEventLog(e, source);
        ...
        public override bool ChangePassword(string username, string oldPwd, string newPwd)
        {
            ...
            User _user = _userProvider.Get(username, pApplicationName).Data;
            _user.Password = EncodePassword(newPwd);
            _user.LastPasswordChangedDate = DateTime.Now;
            return saveUser(_user, "ChangePassword");
        }
                      ...
                      string newPwdAnswer)
        {
            ...
            User _user = _userProvider.Get(username, pApplicationName).Data;
            _user.PasswordQuestion = newPwdQuestion;
            _user.PasswordAnswer = EncodePassword(newPwdAnswer);
            return saveUser(_user, "ChangePasswordQuestionAndAnswer");
        }
                 ...
                 out MembershipCreateStatus status)
        {
            ...
            if (u == null)
            {
                ...
                User _user = new User();
                _user.Password = EncodePassword(password);
                _user.Email = email;
                _user.PasswordQuestion = passwordQuestion;
                _user.PasswordAnswer = EncodePassword(passwordAnswer);
                _user.IsApproved = isApproved;
                _user.Comment = string.Empty;
                _user.LastPasswordChangedDate = createDate;
                _user.LastActivityDate = createDate;
                //_user.ApplicationName = pApplicationName;
                _user.IsLockedOut = false;
                _user.LastLockedOutDate = createDate;
                //_user.FailedPasswordAttemptCount = 0;
                //_user.FailedPasswordAttemptWindowStart = createDate;
                //_user.FailedPasswordAnswerAttemptCount = 0;
                //_user.FailedPasswordAnswerAttemptWindowStart = createDate;
                //_user.Roles = string.Empty;

                try
                {
                    if (_userProvider.Save(_user, Engine.DefaultUser))
                    {
        ...
        public override bool DeleteUser(string username, bool deleteAllRelatedData)
        {
            ...
            try
            {
                if (_userProvider.Delete(_user, Engine.DefaultUser))
                {
        ...
        public override MembershipUserCollection GetAllUsers(int pageIndex, int pageSize, out int totalRecords)
        {
            ...
            try
            {
                foreach (User _loopUser in _userProvider.Get(pApplicationName).Data)
                {
                    users.Add(GetUserFromReader(_loopUser));
                    totalRecords++;
                }
            }
            catch (Exception e)
        ...
        public override MembershipUser GetUser(string username, bool userIsOnline)
        {
            ...
            try
            {
                if (_user != null)
                {
                    u = GetUserFromReader(_user);
                    if (userIsOnline)
                    {
                        _user.LastActivityDate = DateTime.Now;
                        _userProvider.Save(_user, Engine.DefaultUser);
                    }
                }
            }
            catch (Exception e)
        ...
        {
            User _user = _userProvider.Get(Convert.ToInt32(providerUserKey)).Data;
            MembershipUser u = null;
            try
            {
                if (_user != null)
        ...
        {
            User _user = _userProvider.GetByEmail(email, pApplicationName).Data;
        ...
        {
            User _user = _userProvider.Get(user.UserName, pApplicationName).Data;
            try
            {
                if (_user != null)
                {
        ...
        private string EncodePassword(string password)
        {
            ...
            switch (PasswordFormat)
            {
                ...
                case MembershipPasswordFormat.Hashed:
                    ...
                    hash.Key = HexToByte(ConfigurationManager.AppSettings["ValidationKey"]); //  machineKey.ValidationKey
