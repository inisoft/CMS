...
{
    public partial class Delete : BaseViewControl<ArticleModel>
    {
        ...
        protected override void OnSMCSPostBack()
        {
            ...
            if (Html.IsValid)
            {
                switch (URLContext.Action.ToLower())
                {
                    case "delete":
                        BL.Repositories.Article _repository = new BL.Repositories.Article();
                        bool _result = _repository.Delete(Model.ToEntity());
                        if (_result)
                        {
                        ...
                        {
                            this.Html.AddValidationResult(Model, "Nie można skasować danych");
                        }
                        break;
                }
            }
